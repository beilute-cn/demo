...
def real_time_dashboard(file_path, duration=60):
    ...
    try:
        ...
        print("提示: 更新期间请避免手动编辑Excel，或等待重试...")
        ...
        while time.time() - start_time < duration:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if 1 or e.args[0] == -2147418111:  # Call was rejected by callee
                        if attempt < max_retries - 1:
                            print(f"  Excel正忙，等待重试... ({attempt + 1}/{max_retries})")
                            time.sleep(retry_delay)
                        else:
                            print(f"  跳过行 {row}（Excel持续忙碌）")
                    else:
                        raise  # 其他错误直接抛出
